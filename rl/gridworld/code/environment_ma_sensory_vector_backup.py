...
class Env(tk.Tk):
    def __init__(self, num_agents=2, num_obstacles=2, obstacles_random_steps=20, is_agent_silent=False, sensory_size=3):
    ...
    def _build_canvas(self):
        ...
        for agent in self.agents:
            ...
            agent['image_obj'] = canvas.create_oval(agent_center_x - UNIT / 4, agent_center_y - UNIT / 4,
                                                    agent_center_x + UNIT / 4, agent_center_y + UNIT / 4,
                                                    fill='blue', outline='black')

        # Initialize obstacles at specific locations
        obstacle_positions = [
            (4.5 * UNIT, 1.5 * UNIT),  # Coordinates for the first obstacle
            (1.5 * UNIT, 4.5 * UNIT),  # Coordinates for the second obstacle
            (1.5 * UNIT, 3.5 * UNIT),  # Coordinates for the third obstacle
            (2.5 * UNIT, 2.5 * UNIT)   # Coordinates for the fourth obstacle
        ]
        ...
        for i in range(self.num_obstacles):
            ...
            obstacle = canvas.create_rectangle(pos[0] - UNIT / 4, pos[1] - UNIT / 4,
                                            pos[0] + UNIT / 4, pos[1] + UNIT / 4,
                                            fill='red', outline='black')
            self.obstacles.append(obstacle)


        # Properly center the target within its grid cell
        self.circle = canvas.create_oval(4.5 * UNIT - UNIT / 4, 4.5 * UNIT - UNIT / 4,
                                 4.5 * UNIT + UNIT / 4, 4.5 * UNIT + UNIT / 4,
    ...
    def reset(self):
        ...
        for agent, pos in zip(self.agents, agent_positions):
            x, y = pos
            self.canvas.coords(agent['image_obj'], x - UNIT / 4, y - UNIT / 4, x + UNIT / 4, y + UNIT / 4)
            agent['coords'] = [x, y]
    ...
    def step(self, actions):
        ...
        for idx, (agent, action) in enumerate(zip(self.agents, actions)):
            ...
            self.canvas.coords(agent['image_obj'], new_coords[0] - UNIT / 4, new_coords[1] - UNIT / 4,
                               new_coords[0] + UNIT / 4, new_coords[1] + UNIT / 4)
