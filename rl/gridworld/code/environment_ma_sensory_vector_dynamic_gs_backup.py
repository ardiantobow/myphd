...
class Env(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        min_distance_from_agents = 0  # Set a smaller minimum distance from agents (closer proximity)
        max_distance_from_agents = 3  # Set a max distance to ensure obstacles are close enough to be real obstacles
        obstacle_positions = []
        ...
        for i in range(self.num_obstacles):
            while True:
                # Generate obstacle positions near the agents within a defined range
                agent_index = np.random.randint(0, len(self.agents))  # Pick a random agent to base the obstacle position on
                agent_x, agent_y = agent_positions[agent_index]
                
                x_offset = np.random.randint(-max_distance_from_agents, max_distance_from_agents + 1) * self.UNIT
                y_offset = np.random.randint(-max_distance_from_agents, max_distance_from_agents + 1) * self.UNIT

                x = agent_x + x_offset
                y = agent_y + y_offset
                pos = (x, y)
                
                # Ensure the obstacle is within bounds, far enough from agents and not on the target position
                if (self.UNIT / 2 <= x <= (self.WIDTH - 0.5) * self.UNIT and
                    self.UNIT / 2 <= y <= (self.HEIGHT - 0.5) * self.UNIT and
                    np.linalg.norm(np.array(pos) - np.array((agent_x, agent_y))) >= min_distance_from_agents * self.UNIT and
                    pos != target_position and
                    all(np.linalg.norm(np.array(pos) - np.array(agent['coords'])) > min_distance_from_agents * self.UNIT for agent in self.agents)):
