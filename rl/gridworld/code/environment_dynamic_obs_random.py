...
class Env(tk.Tk):
    ...
    def reset(self):
        ...
        self.canvas.delete(self.triangle1)  # Delete the previous obstacle
        x_obs1 = np.random.randint(0, WIDTH) * UNIT + UNIT / 2  # Random x coordinate
        y_obs1 = np.random.randint(0, HEIGHT) * UNIT + UNIT / 2  # Random y coordinate
        self.triangle1 = self.canvas.create_image(x_obs1, y_obs1, image=self.shapes[1])  # Create new obstacle
    ...
    def step(self, action):
        ...
        if triangle1_state[0] <= 0 or triangle1_state[0] >= (WIDTH - 1) * UNIT:  # Check boundaries
